...
import (
	...
	"github.com/nekishdev/graphql-catalog/golang/db"
	"github.com/nekishdev/graphql-catalog/golang/gorm_models"
	"github.com/nekishdev/graphql-catalog/golang/graph/generated"
	"github.com/nekishdev/graphql-catalog/golang/graph/model"
)
...
func (r *mutationResolver) CreateCategory(ctx context.Context, input model.CategoryFillable) (*gorm_models.Category, error) {
...
func (r *mutationResolver) UpdateCategory(ctx context.Context, id uint, input model.CategoryFillable) (*gorm_models.Category, error) {
	var (
		err      error
		category gorm_models.Category
	)
	...
	if err = db.GetDB().First(&category, id).Error; err != nil {
		return nil, fmt.Errorf("failed to found category \"%d\": %s", id, err)
	}

	if err = db.GetDB().Model(&category).Update(input).Error; err != nil {
		return nil, fmt.Errorf("failed to update category with id \"%d\": %s", id, err)
	}
...
func (r *mutationResolver) DeleteCategory(ctx context.Context, id uint) (bool, error) {
	...
	err := db.GetDB().
		Unscoped().
		Select("Products").
		Delete(&gorm_models.Category{ID: id}).
		Error
	if err != nil {
		return false, fmt.Errorf("failed to delete category with id \"%d\": %s", id, err)
	}
	return true, nil
}
...
func (r *mutationResolver) DeleteProductProperty(ctx context.Context, code string) (bool, error) {
	err := db.GetDB().Select("Values").Delete(gorm_models.ProductProperty{Code: code}).Error
	if err != nil {
		return false, fmt.Errorf("failed to delete product property with code \"%s\": %s", code, err)
	}
	return true, nil
}
